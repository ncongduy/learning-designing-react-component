...
export default async function handler(req, res) {
		...
		try {
			...
			if (!speakers) {
				...
				const newSpeakerArray = speakers.map((rec) =>
					parseInt(rec.id) === id ? recordFromBody : rec
				);
				writeFile(
					jsonFile,
			...
			res.status(500).send(
				`PUT /api/speakers/${id} status: 500 unexpected error`
			);
	...
	async function postMethod() {
		try {
			...
			res.status(500).send(
				`POST /api/speakers/${id} status: 500 unexpected error`
			);
	...
	async function deleteMethod() {
		try {
			...
			if (!speakers) {
				...
				const newSpeakerArray = speakers.filter(
					(rec) => parseInt(rec.id) !== id
				);
				writeFile(
					jsonFile,
					JSON.stringify({ speakers: newSpeakerArray }, null, 2)
